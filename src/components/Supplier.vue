<template>
  <el-container class="supplier">
    ...
    <el-row class="top">
      <!-- Create new button -->
      <el-col :span="7">
        <el-button type="text" @click="navigateTo('/NewSupplier')" class="create-new">
          Create new
        </el-button>
      </el-col>
      ...
      <el-col :span="4">
        <el-input v-model="searchQuery" placeholder="Enter Supplier ID..." class="input-field"></el-input>
      </el-col>
      <el-col :span="1">
        <el-button slot="append" icon="Search" @click="searchSupplier"></el-button>
      </el-col>
      <el-col :span="4">
            ...
            <el-col :span="24">
              <el-form label-position="left" label-width="118px">
                <el-form-item label="Supplier Name :">
                  <el-input v-model="filters.keyword1" placeholder="Enter keyword..."></el-input>
                </el-form-item>
                <el-form-item label="Country :">
                  <el-input v-model="filters.keyword2" placeholder="Enter keyword..."></el-input>
                </el-form-item>
                <el-form-item label="Contact Info :">
                  <el-input v-model="filters.keyword3" placeholder="Enter keyword..."></el-input>
                </el-form-item>
                <el-form-item label="User ID :">
                  <el-input v-model="filters.keyword4" placeholder="Enter keyword..."></el-input>
                </el-form-item>
                <el-button type="primary" plain @click="filterResults" style="margin-left: 200px;">Filter</el-button>
              </el-form>
            </el-col>
          </el-row>
          <br/>
          <div class="result-text">Results:</div>
          <el-table :data="tableData" @row-dblclick="viewDetails" highlight-current-row @current-change="handleCurrentChange">
            <el-table-column prop="supplierID" label="Supplier ID"></el-table-column>
            <el-table-column prop="suppliername" label="SupplierName"></el-table-column>
        ...
        <el-tabs v-model="activeTab">
          <el-tab-pane label="Common" name="common">
            <div>
              ...
              <span class="text-value">{{ selectedSupplier.supplierID || '000000' }}</span>
              <span class="text-label">Supplier Name :</span>
              <span class="text-value">{{ selectedSupplier.suppliername || '000000' }}</span>
              <span class="text-label">BP Role :</span>
              <span class="text-value">{{ selectedSupplier.partnerFunctions || '000000' }}</span>
              <p class="text-title">Address</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label">Street Address :</span>
                <span class="text-value">{{ selectedSupplier.streetAddress || '000000' }}</span>
                <span class="text-label">Postal Code :</span>
                <span class="text-value">{{ selectedSupplier.postalCode || '000000' }}</span>
              <p>
                <span class="text-label">Country :</span>
               <span class="text-value">{{ selectedSupplier.country || '000000' }}</span>
               <span class="text-label">Region :</span>
               <span class="text-value">{{ selectedSupplier.region || '000000' }}</span>
               <span class="text-label">City :</span>
               <span class="text-value">{{ selectedSupplier.city || '000000' }}</span>
              </p>
              <p class="text-title">Communication</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label">Language :</span>
              <span class="text-value">{{ selectedSupplier.communicationLang || '000000' }}</span>
              <p class="text-title">Contact :</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label">Contact Info :</span>
              <span class="text-value">{{ selectedSupplier.contactInfo || '000000' }}</span>
              <span class="text-label">User ID:</span>
              <span class="text-value">{{ selectedSupplier.userID || '000000' }}</span>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="Vendor & Account & Payment" name="vendor">
            <div>
              ...
              <span class="text-value">{{ selectedSupplier.taxNumber || '000000' }}</span>
              <p class="text-title">Account Management</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label1">Reconciliation Acct :</span>
              <span class="text-value">{{ selectedSupplier.reconciliationAcct || '000000' }}</span>
              <p class="text-title">Payment Data</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label1">Terms of Payments :</span>
              <span class="text-value">{{ selectedSupplier.termsOfPayment || '000000' }}</span>
              <span class="text-label1">Check Double Invoice :</span>
              <span class="text-value">{{ selectedSupplier.checkDoubleInvoice || '000000' }}</span>
              <p class="text-title">Correspondence</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label1">Clerk at Vendor :</span>
              <span class="text-value">{{ selectedSupplier.clerkName || '000000' }}</span>
            </div>
          </el-tab-pane>
          ...
          <el-tab-pane label="Purchasing" name="purchasing">
            <div>
              ...
              <span class="text-value">{{ selectedSupplier.purchasingOrg || '000000' }}</span>
              <p class="text-title">Purchase Data</p>
              <el-divider class="divider"></el-divider>
              <span class="text-label1">Order Currency :</span>
              <span class="text-value">{{ selectedSupplier.orderCurrency || '000000' }}</span>
            </div>
          </el-tab-pane>
        </el-tabs>
        <br/>
...
export default {
    ...
    async function searchSupplier() {
      try {
        ...
        if (!searchQuery.value) {
          ElMessageBox.alert('Please enter a Supplier ID before searching.', 'No Input', {
  ...
  try {
    if (!filters.value.keyword1 && !filters.value.keyword2 && !filters.value.keyword3 && !filters.value.keyword4) {
      ElMessageBox.alert('Please enter at least one filter criterion.', 'No Filters', {
        confirmButtonText: 'OK',
        type: 'warning',
      });
      return;
    ...
    if (response.data.code === 1 && response.data.data.length > 0) {
      ...
      ElMessageBox.alert('No results found with the given filters.', 'Filter Results', {
        confirmButtonText: 'OK',
        type: 'warning',
      });
      tableData.value = [];
  ...
  if (selectedSupplier.value) {
    ...
    ElMessageBox.alert('Please select a supplier to modify.', 'No Supplier Selected', {
      confirmButtonText: 'OK',
      type: 'warning',
    });
  }
