<template>
    ...
    <div class="total">
        <div class="main">
            <el-button type="primary" plain class="createButton"><RouterLink to="/create">Create New</RouterLink>></el-button>
            <div class="search-container">
                <div class="search">
                    <span class="infoText">Standard PO :</span>
                    <el-input placeholder="请输入内容" v-model="searchPO" class="input-with-select"></el-input>
                    <!-- 图标同样无法显示 -->
                    <el-button @click="find" slot="append">Find</el-button>
                        ...
                        <!-- 查找结果弹出框 -->
                        <el-dialog v-if="found" v-model="result" title="Find Purchasing Order(result)" width="500">
                            <el-table :data="tableData" @row-click="detailed">
                                <el-table-column prop="searchTerm" label="Search Term" width="180"></el-table-column>
                                <el-table-column prop="POnumber" label="PO Number" width="180"></el-table-column>
                            </el-table>
                        </el-dialog>
                        <!-- 失败提示框 -->
                        <el-dialog v-else v-model="result" title="Find Purchasing Order(result)" width="500">
                            查找失败
                        </el-dialog>

                        <!-- 查看详情界面（detail）弹出框 -->
                        <el-dialog v-model="detail" title="Detail" width="98vw">
                            <span>PO Number:{{ poDetail }}</span>
                            <el-table :data="poData" class="table">
                                <el-table-column prop="materialId" label="Material ID" width="180" />
                                ...
                                <el-table-column prop="purchasingGroup" label="Purchasing Group" width="180" />
                                <el-table-column prop="purchasingOrganization" label="Purchasing Organization" width="180" />
                                <el-table-column prop="plant" label="Plant" width="180" />
                                <el-table-column prop="paymentTerms" label="Payment Terms" width="180" />
                            </el-table>
                        </el-dialog>

                    <el-button type="info" plain class="info-button"><RouterLink to="/modify">Modify</RouterLink>></el-button>
                </div>
            </div>
        </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    go(sterm){//query By UserID(search term)
        this.result = true
        axios.get('/purchase_order/query', {
