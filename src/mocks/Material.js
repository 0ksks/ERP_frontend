...
// Mock for GET /material/query
    ...
    mock.onGet('/api/material/query_success').reply(config => {
        ...
        const { materialID, materialName, description, materialGroup, userID } = params;
  ...
  if (materialName) {
    filteredData = filteredData.filter(item => item.materialName.includes(materialName));
  }
  if (description) {
    filteredData = filteredData.filter(item => item.description.includes(description));
  }
  if (materialGroup) {
    filteredData = filteredData.filter(item => item.materialGroup.includes(materialGroup));
  }
  if (userID) {
