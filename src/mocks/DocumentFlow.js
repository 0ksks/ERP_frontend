...
mock.onPost('/document_flow/display').reply(config => {
  ...
  if (purchaseOrderID) {
    filteredData = filteredData.filter(item => item.purchaseOrderID.includes(purchaseOrderID));
  }
  if (goodsReceiptID) {
    filteredData = filteredData.filter(item => item.goodsReceiptID.includes(goodsReceiptID));
  }
